...
* 1 solution to build desktop applications: {electron}
* 1 mobile framework: {react-native}
* 1 CSS toolkit: {bootstrap}
* 1 state management library based on functional concepts: {redux}
* 1 powerful and flexible chart library: {d3}
...
{react} is number 4, no front-end developer can ignore React and its rich eco-system.

But what is more impressive is the rise of {react-native}.
...
const framework = `
We have already mentioned {vuejs} (number 1 overall) and {react} (number 3), so let's have a look at the following frameworks.
...
const ide = `
Talking about desktop applications, it's worth to mention that 2 of the most popular IDEs are open-source projects made with web technologies.

* In our results, Microsoft leads the way with {vscode} that provides a nice integration with TypeScript and node.js. Saying "open source" and "Microsoft" in the same sentence is no more an oxymore!
* {atom}, pushed by Github, is not far behind VSCode. A fun fact about Atom: its main language is CoffeeScript!
`
...
* The most trendy transpiler was {typescript}, it brings to web developers the static types used by Java and C# developers . The fact that {angular-2} uses TypeScript added even more traction. There are pros and cons about using types in JavaScript, read this article [You Might Not Need TypeScript](https://medium.com/javascript-scene/you-might-not-need-typescript-or-static-types-aa7cb670a77b#.1pn05vlis)
...
* During years {coffeescript} and its lean syntax, inspired by Python and Ruby syntax, was the most popular compiler but it was less more trendy in 2017, a lot of developers moved from CoffeeScript to ES6 with Babel.
`
...
const reacttemplate = `
React is a great UI library, but it's only the UI layer... so how to start real-world application ?
It's the answer provided by the React "boilerplates" and other "starter kits".

* The well named {react-boilerplate} has every thing you need, including {redux} and some nice offline features.
...
* {hexo} is a thorough SSG that can be used to build a blog for example, it has a lot of features including an internationalization plugin.
* The newcomer {gatsby} is a very interesting solution, it stands out from its condenders because it uses React eco systen to generate static html files. The fact that you can combine React components, mardown files and server-side rendering makes it very powerful.
`
